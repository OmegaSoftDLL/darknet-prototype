...
- **Fix CI — modo `--headless`**: no runner do GitHub Actions (Windows Server 2025, sem display/GPU) o validate estourava `Segmentation fault` (exit 139) ao criar o contexto GL do raylib, ANTES de imprimir qualquer INFO. Solucao: flag `--headless` setando `Game::headless` em main.cpp ANTES do construtor — o construtor pula InitWindow/render textures/SpriteBank/modelos/postfx/lightSystem/background (mantem `audio.init()` sem device + loadPhaseDefs/buildQuests/buildNPCs + mundo aberto e fase), `runHeadless()` roda a MESMA `update()` em tempo real (dt proprio do loop, sem render/presentFrame/screenshot/menu), FPS do bot medido do proprio loop (`GetFPS()`=0 sem janela), destructor so libera o criado. ci.yml passa `--headless` nas duas validates; validate.sh ganha 3o argumento (headless=1). Gate local headless: Release+Debug exit 0, testes 12/12 (109), validates APROVADOS (bot 21 abates, 4/4 quadrantes, 1 fase avancada pelo portal, 0 mortes, sem stuck).
