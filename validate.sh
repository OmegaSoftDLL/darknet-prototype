...
# nothing) and runs the bot with the fixed seed. Exits non-zero if the build is
# not playable.
#   usage: ./validate.sh [seconds] [seed] [headless]
#   headless=1 runs without window/GPU (same mode the CI)
# ------------------------------------------------------------------------------
set -u
SECS="${1:-100}"
SEED="${2:-20260821}"
...
CODE=$?
grep -E "VALIDATION|PHASES:|POSTFX|WORLDLIT" "$ROOT/build/Release/validate.log" | head -20
if [ "$CODE" -ne 0 ]; then
  echo "REJECTED (exit $CODE) - reasons above"; FAIL=1
else
  echo "APPROVED"
fi
exit $FAIL
