...
2. **Build Debug and Release.** `validate.sh` already does both. The project owner plays **Release** — a delivery validated only in Debug already cost a full day of "I don't see anything that was done".
...
- **Where to change:** `Game.cpp:4252` (bot receives `tilemap.portals`, empty in
  open world; needs to know `owPortalPos` when `owPortalOpen`) and the simulation of `[E]` (today `IsKeyPressed(KEY_E)` in `updatePhasePortal` — the bot does not press keys; give the bot decision a path to trigger advancement when <105u from portal).
...
- **Acceptance:** screenshot of the hub with no clay roof/tower castle from `BuildingSystem` in urban zones (LARuins/GhostCity). In zones where castle is coherent (Manor), it may stay.
...
- **Two known causes, attack in cost order:**
  ...
  2. **Real:** `SpriteExtrude::CaptureToImage` creates a RenderTexture per capture
     ...
     transition screen, reuse a single RenderTexture, or cache meshes.
...
- **Acceptance:** in Cemetery phase (3rd… check `content/phases.txt`), the hub
  uses the biome's catalog (crypts/towers, not modern buildings). Verifiable by screenshot as soon as item #1 grants access to phases 2+.
...
- **Acceptance:** `bot_report.txt` with `Melee attacks > 10` in a 120 s run, or
...
- **Scale:** ruler measured in code — hero = **66u** tall (log
  ...
  never "by eye" (scale oscillation consumed a whole afternoon).
- **New overlay** goes in `drawHudAndOverlays`/`drawUI` (shared) — never only in one render path (historical root of "fix 1, break 30").
- **Ground shadow/decal:** layer separation in tenths of a unit, not
  0.004 (z-fighting made the whole scenery flicker).
- **2D shadow ellipse** without `g_renderPass3D` guard becomes a pedestal in the voxelized mesh.
...
Ideal delivery: both `validate.sh` green, `Advanced zones ≥ 1`, and a paragraph
