...
### [P0] Phase portal can spawn inside the building — progression soft-lock
...
> (P2: make guarantee intentional). Text below is kept the the record of the
...
- **Evidence:** **DEDUCED from code, not observed in execution.** Scenery is
  ...
  solid tiles with radius `125 × scale` (`markSolidAt`); portal interaction requires the player to be **105 u** from center (`Game.cpp:5491`). If the building occupies the point, there is in the way to get close enough: **the phase never closes**.
...
- **Player effect:** dead progression with in the error message. The kind of bug that
  escapes all manual testing and becomes the negative review.
...
- **Evidence:** **MEASURED** — `Advanced zones: 0` in all validated runs to date.
  ...
  central game mechanic is the only one with in the safety net; phases 2–11 are,
...
- **Where:** `src/Game.cpp:8048` (text) vs. all uses of `owPortalPos`
  (lines 5486, 5491, 7077, 7102 — spawn, proximity, draw, light; **in the
  directional indicator**).
- **Evidence:** **MEASURED** — frame `shot_09` (seed 20260821, t≈99 s) shows
  "PORTAL OPEN — follow the marker and press [E]" with in the arrow, in the radar blip,
  nothing pointing the way. The visible pink arrows are off-screen enemy indicators.
- **Effect:** the player has to guess the direction of an unsignaled point in the
  world of radius 3000+. The game's own instruction lies.

### [P1] Player base is the medieval castle in the modern city
...
- **Cause (DEDUCED):** voxel model generation at match entry.
  `SpriteExtrude::CaptureToImage` creates the RenderTexture **per capture** and does
  `LoadImageFromTexture` (GPU→CPU readback = pipeline stall). ~40+ models
  (types × 4 walk poses), amortized at 3 per frame (`m_voxGenBudget`) — creates the
...
- Cars read the cars: wheels, recessed cabin, fenders (`shot_09`).
...
- **Save/load** (roundtrip of continuing the match).
...
- **Phases 2–11 in real execution** — in the run reached there (direct consequence
...
1. **Portal at the guaranteed free point** — validate/relocate `owPortalPos`
   (or clear solids in the radius when opening). Kills the P0; the the bonus, the future
   marker will have the reliable target.
...
> update, world and UI). Treat any change there the high regression risk and run
> `./validate.sh 120 7` **and** the second seed before committing.
...
I ported the region (0,0) generator to the measurement script (scratchpad, outside the
...
> again if: portal offset changes, `EDGE`/grid changes, catalogs gain footprint > 105u near the hub, or interaction radius shrinks. Structural protection is accidental — in the line of code guarantees the corridor free; the guard comment in code (or an `!isBlocked` assert when opening the portal) would make the guarantee intentional. Kept the P2 recommendation.
...
- **Evidence (DEDUCED from code):** `setupWorldRegions` always rebuilds the same 3×3 map; region (0,0) is always `LARuins`, and the city grid branch uses `structuresFor(r.zoneType)` = urban catalog — in **every** phase. The floor follows `biomeAtWorld` = phase zone. Expected result: in Cemetery phase, the hub will have modern buildings and bunkers over cemetery ground. Not observable today because in the run reaches phases 2+ (P1 coverage finding) — the two pending items add up.

### Declared method limit

The replica did not reproduce the binary's exact stream (~0.7% checksum difference). The portal conclusion does not depend on this (120 uncorrelated variants agree), but any future use of the replica for questions *sensitive to the exact position* of the specific object requires closing that gap first.
...
| P1 | "Follow the marker" with in the marker | Open | Measured: frame `shot_09` |
...
2. **Portal marker on HUD/radar** — or fix the text that promises the
   nonexistent marker. Along with it, the `!isBlocked(owPortalPos)` assert on open
   (3 lines, turns the accidental §6 protection into the guarantee).
...
4. **Opening hitch** — eliminate double scenery build (free win, measured) and removes GPU→CPU readback from the hot path of voxel generation (pre-generate on title/transition screen, or cache meshes on disk).

### Repository hygiene observation

During the audit the modification was found in the working tree in
...
declares the `reset()` method referring to the `Game::restartRun` that does **not exist
