...
3. **Save is not reliable.** Zeroed metadata, lost equipment, residual state between matches, no checksum/backup.
...
- **Description:** Files `Game_Gameplay.cpp`, `Game_WorldRender.cpp`, etc. are physical partitions of the same class. All `#include "Game.h"` and freely access private members. There is no real encapsulation.
...
- **Description:** Directly includes `Player.h`, `Enemy.h`, `Item.h`, `Tilemap.h`, `AudioManager.h`, `NetClient.h`, `StoreClient.h`, etc. Forces cascading recompilation and creates a bloated header.
...
- **Recommendation:** Centralize in `World/BiomeUtils.h` and `World/BuildingMetrics.h` as `inline constexpr`.
...
- **Recommendation:** Add per-compiler flags and treat warnings as errors in CI.
...
- **Recommendation:** Remove manual knockback from the specific updates.
...
- **Recommendation:** Use moving average, e.g. `* 0.6f + dpm * 0.4f`.
...
- **Description:** Player level + global scaling (cap 4x) + difficulty + threat level (no cap) + mutator + ring + phase. Enemies can reach 50x–100x HP/damage.
- **Recommendation:** Apply a global combined scaling cap or make part of it additive.
...
- **Description:** On every new entity type/pose the game captures a 2D sprite, creates outline, crops **and generates a real 3D `Model` via `SpriteExtrude::BuildVoxelModel`**. The comment confirms: *"Real 3D voxel: cached but NOT rendered"*. Each pose generates a GPU mesh, occupying VRAM and causing stalls, never used.
- **Recommendation:** Remove `BuildVoxelModel` from `ensureVoxel` flow if not used.
...
- **Recommendation:** Sort entities by depth and use a single Begin/End state-change pair.

#### P1 — Enemies/bosses emit 50–100 draw calls each
- **File:** `src/Enemy_Render.cpp` (e.g. `renderBossAura`, `renderOrcCibernetico`, `renderOmegaBoss`)
- **Description:** Each enemy drawn with dozens of individual primitives. No batching/instancing.
- **Recommendation:** Render in a single atlas/texture atlas or accumulate geometry in a CPU-side vertex buffer.
...
- **Recommendation:** Split into two phases: normals first, then additive in a single Begin/End.

#### P1 — Scanlines as hundreds of rectangles
...
- **Description:** `startThread()` stacks `std::thread` in `threads_` with no concurrency limit.
...
- **Description:** Code comments "Accept any DARKNET_SAVE_V* version", but there is no per-version migration.
...
- **Description:** Auto-save every 30s and on every zone transition, with no guard against active combat, transition, dead player, dialogue, or pause.
...
- **Recommendation:** Remove multi-slot API or integrate it into the main menu.
...
- **Description:** `spawnWave` receives `zoneW/zoneH` from tilemap (24,576×24,576 units) and places fixed candidates in that rectangle. The real open world is a disk of `owPhaseRadius` (3,000–5,000). Most candidates lie beyond the phase barrier.
- **Recommendation:** Generate candidates inside circle `safeZoneCenter + owPhaseRadius - margin`.

#### P0 — Player does not do separate X/Y sliding
...
- **Recommendation:** Test `isBlocked(portalSpawnPos)` and shift radially until a free point is found.
...
- **Recommendation:** Unify into a single query function.
...
- **Recommendation:** Call `srand((unsigned)time(nullptr))` at start of `main()` when no `--seed`; migrate `rand()` to `GetRandomValue`.

#### P0 — `TutorialSystem` is not instantiated or used
- **File:** `src/Game.h` (absence), `src/TutorialSystem.cpp`
- **Description:** The class exists, but **there is no `TutorialSystem` instance in `Game`**. Entire system is dead code.
...
#### P0 — `AchievementSystem` is initialized, but no events are reported
- **File:** `src/Game.cpp:245`, `src/Achievement.cpp:131-206`
- **Description:** `achievements.init()` is called, but no `achievements.onKill`, `onLevelUp`, `onPortalClosed`, etc. is invoked.
...
#### P0 — Keys hardcoded; no rebind system
...
- **Recommendation:** Separate sprint from RTS selection (e.g. ALT/CTRL).
...
- **Description:** Synthesized death sound is loaded, occupies memory and init time, but has no caller.
- **Recommendation:** Remove or replace `playDeathCry()` with `playPlayerDeath()`.
...
- **Recommendation:** Remove unused assets/methods or connect to events.
...
- **Recommendation:** Standardize to a single PRNG seeded by `--seed` throughout the game.

### Test coverage
- 26 tests cover `CraftingSystem`, `Enemy`, `SaveManager`, `Player`, `Projectile`, `Tilemap`.
- The `Game` class has no unit tests because of coupling.
...
2. **Connect TutorialSystem and AchievementSystem** or remove them from the game.
...
7. **Standardize randomness:** seed `srand()` or migrate everything to a single PRNG.
...
3. Remove unused voxel model generation.
...
From a stability standpoint, the game passes headless validation gates, which is positive. However, **passing autotest does not equal ready to play**: save corrupts metadata, balance can explode from accumulated scaling, and the procedural world has spawn and collision bugs.
